...
public class Goon implements Ship {
	...
	/**
	 * Creates a new ship at a default position with a default speed, used for
	 * debugging
	 */
	public Goon() {
	 ...
	 */
	public void move() {
		if (sprite.getX() > xOrigin + 50 || sprite.getX() > Gdx.graphics.getWidth() - 100)
			dir = -1; // Reached bound of this side, reverse direction
		else if (sprite.getX() < xOrigin - 50 || sprite.getX() < 90)
			dir = 1; // Reached bound of this side, reverse direction
	...
	public void collision(Sprite coll) {
		if (Math.abs(sprite.getX() - coll.getX()) < 25 && Math.abs(sprite.getY() - coll.getY()) < 10 && isAlive()) // If
																													// given
																													// sprite
																													// is
																													// within
