...
public class Asteroid {
	...
	public Asteroid(int type, int levelNum, AssetManager manager) {
		...
		if (type == SMALL)
			texture = manager.get("smallAsteroid.png");
		else if (type == MEDIUM)
			texture = manager.get("mediumAsteroid.png");
		else if (type == LARGE)
			texture = manager.get("largeAsteroid.png");
		else if (type == GOLDEN)
			texture = manager.get("goldenAsteroid.png");
	...
	public Asteroid() {
		...
		if (type == SMALL)
			texture = new Texture("Space/smallAsteroid.png");
		else if (type == MEDIUM)
			texture = new Texture("Space/mediumAsteroid.png");
		else if (type == LARGE)
			texture = new Texture("Space/largeAsteroid.png");
		else if (type == GOLDEN)
			texture = new Texture("Space/GoldenAsteroid.png");
	...
	/**
	 * Destroys asteroid, called when collision is detected with either player or
	 * player fire
	 */
	public void destroy() {
		intact = false;
		sprite.setPosition(0, 0);
		// How much is added to score, based on difficulty to shoot (i.e. size) and
		// current level
		if (type == SMALL)
			Space.score += 60 * levelNum;
		if (type == MEDIUM)
			Space.score += 30 * levelNum;
		if (type == LARGE)
			Space.score += 15 * levelNum;
		if (type == GOLDEN)
			Space.score += 300 * levelNum;
