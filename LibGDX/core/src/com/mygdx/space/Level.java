...
private boolean[] aliveArray;
	...
	public void initLevel(int levelNum, Ship ship)
	{
		...
		{
			enemies = new Ship[5];
			for (int i = 0; i<enemies.length; i++) enemies[i] = new Goon(i*200+200, Gdx.graphics.getHeight()-50,5);
		}
		else if (levelNum == 2)
		{
			enemies = new Ship[10];
			for (int i = 0; i<enemies.length; i++) enemies[i] = new Goon(i*100+200, Gdx.graphics.getHeight()-50,5);
		...
		{
			enemies = new Ship[20];
			for (int i = 0; i<10; i++) enemies[i] = new Goon(i*100+200, Gdx.graphics.getHeight()-50,5);
			for (int j = 10; j<20; j++) enemies[j] = new Ram((j-10)*100+200);
		}
		else if (levelNum == 5)
		{
			enemies = new Ship[34];
			for (int i = 0; i<10; i++) enemies[i] = new Goon(i*100+200, Gdx.graphics.getHeight()-50,8);
			for (int j = 10; j<19; j++) enemies[j] = new Goon((j-10)*100+250, Gdx.graphics.getHeight()-100,8);
